...
STORES_TO_CHECK = {
    ...
    "Jyväskylä (Seppälä)": "k-citymarket-jyvaskyla-seppala",import time
import re
import os
import json
import math
    ...
    try:
        ...
        for search_term in product_list:
            try:
                ...
                time.sleep(2) # Hieman enemmän aikaa lataukselle
                
                # --- PARANNETTU KORTTIEN HAKU ---
                cards = page.locator("[data-testid='product-card']").all()
                if not cards: cards = page.locator("article").all()
                
                if not cards:
                    # Debug-tulostus, jos kortteja ei löydy
                ...
                for card in cards:
                    try:
                        ...
                        if final_price is not None:
                            ...
                            found = True
                            print(".", end="", flush=True) # Löytyi!
                            break 
                    except: continue
                
                if not found: print("o", end="", flush=True) # Tuote ei löytynyt korttien joukosta
            except: 
                print("!", end="", flush=True) # Haku epäonnistui kokonaan
                continue
        print("") 
...
def main():
    ...
    with sync_playwright() as p:
        # --- HEADLESS FALSE PÄÄLLE DEBUGGAUSTA VARTEN ---
